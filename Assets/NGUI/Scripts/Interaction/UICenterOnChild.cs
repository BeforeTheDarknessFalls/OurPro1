...
public class UICenterOnChild : MonoBehaviour
{
	...
	public void Recenter ()
	{
		if (mScrollView == null)
		{
			...
			if (mScrollView == null)
			{
				//Debug.Log(GetType() + " requires " + typeof(UIScrollView) + " on a parent object in order to work", this);
				enabled = false;
				return;
			}
			else
			...
			if (mCenteredObject != null && mCenteredObject.transform == trans.GetChild(index))
			{
				...
				if (Mathf.Abs(delta) > nextPageThreshold)
				{
                    ...
                    if (grid != null && grid.sorting != UIGrid.Sorting.None)
                    {
                        ...
                        Debug.Log("Next Page Threshold requires a sorted UIGrid in order to work properly");
				}
			}
		}
